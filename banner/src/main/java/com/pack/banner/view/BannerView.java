...
import com.pack.banner.Indicator;
...
//        View的常量类，包含点击、长按、拖动、滑动等一些view的配置属性
...
//        动画切换特效
        mViewPager2.setPageTransformer(compositePageTransformer = new CompositePageTransformer());
//        注册viewpage滑动监听回调
        mViewPager2.registerOnPageChangeCallback(new OnPageChangeCallback());
//        设置适配器
        mViewPager2.setAdapter(adapterWrapper = new BannerAdapterWrapper());
    ...
    private void startPager(int startPosition) {
        ...
        mViewPager2.setCurrentItem(tempPosition, false);
        ...
        public void run() {
            if (isAutoPlay()) {
                ...
                if (tempPosition == getRealCount() + sidePage + 1) {
                    ...
                    mViewPager2.setCurrentItem(sidePage, false);
                    post(task);
                } else {
                    isBeginPagerChange = true;
                    mViewPager2.setCurrentItem(tempPosition);
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (isAutoPlay() && mViewPager2.isUserInputEnabled()) {
...
//        滚动状态更改时调用。对于发现用户何时开始拖动，何时开始伪拖动，何时自动将寻呼机稳定到当前页面
//        或何时完全停止/空闲
        @Override
...
//            super.onPageScrollStateChanged(state);
            if (state == ViewPager2.SCROLL_STATE_DRAGGING) {
                if (tempPosition == sidePage - 1) {
                    isBeginPagerChange = false;
                    mViewPager2.setCurrentItem(getRealCount() + tempPosition, false);
    ...
    private void initViewPagerScrollProxy() {
        try {
            ...
            ProxyLayoutManger proxyLayoutManger = new ProxyLayoutManger(getContext(), mViewPager2.getOrientation());
    ...
    public BannerView setPageMargin(int tlWidth, int brWidth, int pageMargin) {
        ...
        if (mViewPager2.getOrientation() == ViewPager2.ORIENTATION_VERTICAL) {
            recyclerView.setPadding(mViewPager2.getPaddingLeft(), tlWidth + Math.abs(pageMargin), mViewPager2.getPaddingRight(), brWidth + Math.abs(pageMargin));
        } else {
            recyclerView.setPadding(tlWidth + Math.abs(pageMargin), mViewPager2.getPaddingTop(), brWidth + Math.abs(pageMargin), mViewPager2.getPaddingBottom());
        }
     ...
     *
     * @param attachToRoot true 添加到BannerView布局中
     */
